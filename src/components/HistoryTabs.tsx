...
export default function HistoryTabs({ lessons }: HistoryTabsProps) {
      ...
      <div className="min-h-[400px]">
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {lessons.map((lesson) => (
              <div key={lesson._id || lesson.dayNumber} className="space-y-4">
                {/* Section Header */}
                <div className="flex items-center gap-4">
                  <h3 className="text-lg font-bold text-slate-800">Day {lesson.dayNumber}</h3>
                ...
                {activeTab === "grammar" && lesson.grammar && (
                  <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
                    <h4 className="text-xl font-bold text-slate-800 mb-2">{lesson.grammar.topic}</h4>
                    <div className="bg-blue-50 text-blue-900 px-4 py-3 rounded-lg font-mono text-sm mb-6 border border-blue-100">
                      {lesson.grammar.rule}
                    </div>
                    
                    <div className="mb-6">
                      <h5 className="text-sm uppercase tracking-wider font-bold text-slate-400 mb-2">When to use</h5>
                      <p className="text-slate-700">{lesson.grammar.whenToUse}</p>
                    </div>

                    <div>
                      <h5 className="text-sm uppercase tracking-wider font-bold text-slate-400 mb-3">Examples</h5>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {lesson.vocabulary?.map((item: any, idx: number) => (
                      <div key={idx} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col h-full">
                        <h4 className="text-xl font-bold text-blue-600 mb-1">{item.word}</h4>
  ...
  return (
    <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 hover:shadow-md transition-shadow">
      <div className="flex justify-between items-start mb-2">
        <h4 className="text-lg font-bold text-blue-600">{item.phrase}</h4>
